...
function presentOnPage(item) {
    for (const iterator of item) {
    table.innerHTML += `    
     <tr>
        <td><b>${iterator.movieName}</b></td>
        <td><img src="${iterator.image}" style = "width:200px;height:200px"></td>
        <td>${iterator.linkToMovie}</td>
        <td>${iterator.rating}</td>
        <td>${iterator.synopsis}</td>
        <td>${iterator._id}</td>
        <td>${iterator.date}</td>
    </tr>   `
 } 
